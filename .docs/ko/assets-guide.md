...
.item-box {
  width: 200px;
  height: 200px;
  background-image: url('{{raw::박스 이미지}}');
  background-size: cover;
}
</style>
<div class="item-box">
  <img src="{{raw::아이템 이미지}}" alt="아이템">
</div>
...
- 같은 이름의 에셋이 여러 개 있으면 랜덤으로 사용됩니다
...
캐릭터 icon을 변경하거나, 캐릭터 카드를 export/import할 때 시스템이 자동으로 관리하는 메타데이터입니다.
...
## 주의사항
- 웹에서 에셋 저장 시 파일 이름이 같을 경우 덮어쓰기가 발생할 수 있으니 주의하세요.
- 웹에서 에셋을 삭제하더라도 assets/ 폴더 내 파일은 남아 있을 수 있습니다.
