...
> **중요**: `onStart`는 `onInput`보다 **권한이 강합니다**. `onInput`이 작동하지 않을 경우 `onStart`를 사용해보세요.
...
-- 사용자 입력 받기
local input = alertInput(id, "이름을 입력하세요:"):await()
print("User entered: " .. input)

-- 선택지 표시 (반환값: 선택된 인덱스, 1부터 시작)
local choice = alertSelect(id, {"옵션 1", "옵션 2", "옵션 3"}):await()
if choice == 1 then
    print("User selected option 1")
end

-- 확인 대화상자
local confirmed = alertConfirm(id, "정말로 실행하시겠습니까?"):await()
if confirmed then
...
setCharacterFirstMessage(id, "안녕하세요!"):await()
...
### 2. ❌ `stopChat()` 함수와 `setDescription()` 사용 불가

`stopChat(id)` 함수와 `setDescription(id, "설명")` 함수는 버그로 인해 **사용할 수 없습니다**. 
...
- `print()` 디버깅을 적극 활용해 코드 실행이 막히는 부분을 확인해보세요.
...
**추가 참고**: 모든 전역 함수 목록은 [`public/lua/risuai-types.lua`](/public/lua/risuai-types.lua) 를 확인하세요.
