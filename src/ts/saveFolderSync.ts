        ...
        watchInterval = setInterval(async () => {
          ...
          try {
            ...
            if (mtime > lastMtime) {
              ...
              // Proxy의 set trap이 currentSaveFolderBot도 업데이트하고
              // 실제 배열에도 저장하여 Svelte가 변경을 감지할 수 있게 함
...
async function saveChangesToFiles(
  ...
  for (const changeStr of changes) {
    ...
    // 예외 처리: chats 관련 데이터는 임시로 비활성화
    // if (jsonPointer.startsWith('/chats')) {
    //   console.log(`[Save Folder] TEMPORARILY DISABLED: ${jsonPointer}`);
    //   continue;
    // }
...
if (typeof window !== 'undefined') {
  ...
  currentSaveFolderBot.subscribe((currentBot) => {
    if (currentBot) {
      ...
      previousData = JSON.parse(JSON.stringify(currentBot.character));
